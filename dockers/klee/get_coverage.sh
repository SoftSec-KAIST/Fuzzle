...
gcc -fprofile-arcs -ftest-coverage -o gcov-bin $TARGET_DIR/$TARGET'_cov.c' /home/maze/klee_build/lib/libkleeRuntest.so
mkdir $TC_DIR/cov_txt_$MAZE_TOOL $TC_DIR/cov_gcov_$MAZE_TOOL

if [ $NUMB_HOURS -lt 1 ]
then
	for tc in $TC_DIR/*tc.ktest
	do
		KTEST_FILE=$tc ./gcov-bin
	done

	for crash in $TC_DIR/*crash.ktest
	do
		KTEST_FILE=$crash ./gcov-bin
		break
	done

	gcov -b -c -s $TARGET_DIR $TARGET'_cov.c' > $TC_DIR/cov_txt_$MAZE_TOOL/$MAZE_TOOL.txt
	ls $TC_DIR/*crash.ktest | head -1 >> $TC_DIR/cov_txt_$MAZE_TOOL/$MAZE_TOOL.txt
...
do
	...
	ls $TC_DIR/*crash.ktest | head -1 >> $TC_DIR/cov_txt_$MAZE_TOOL/$MAZE_TOOL'_'$HOUR'hr.txt'
