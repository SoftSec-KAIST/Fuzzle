...
CP_BT_CMD = 'docker cp %s:/home/maze/outputs/triage %s'
...
def get_src_path(algo, width, height, seed, num, cycle, gen):
    gen = gen.split('-')[0] + '_gen'
...
def get_target_path(algo, width, height, seed, num, cycle, gen):
    target_path = '/home/maze/maze/targets/%s_%sx%s_%s_%s_%s_%s.txt' % \
        (algo, width, height, seed, num, cycle, gen)
    return target_path
...
def run_tools(conf, works):
    for i in range(len(works)):
        ...
        script = '/home/maze/tools/run_%s_multi.sh' % tool
        src_path = get_src_path(algo, width, height, seed, num, cycle, gen)
        bin_path = get_bin_path(algo, width, height, seed, num, cycle, gen)
        target_path = get_target_path(algo, width, height, seed, num, cycle, gen)
        duration = conf['Duration']
        cmd = '%s %s %s %s %s' % (script, src_path, bin_path, duration, target_path)
...
def run_triage(out_dir, works):
    # First, obtain backtrace using gdb
    for i in range(len(works)):
        algo, width, height, seed, num, cycle, gen, tool, epoch = works[i]
        if tool == 'afl++':
            tool_ = 'aflpp'
        ...
        cmd = '/home/maze/tools/run_triage.sh'
        run_cmd_in_docker(container, cmd)

    time.sleep(60)

    # Next, store trace to host filesystem
    for i in range(len(works)):
        algo, width, height, seed, num, cycle, gen, tool, epoch = works[i]
        if tool == 'afl++':
            tool_ = 'aflpp'
        ...
        out_path = os.path.join(out_dir, maze, '%s-%d' % (tool, epoch))
        os.system('mkdir -p %s' % out_path)
        cmd = CP_BT_CMD % (container, out_path)
        run_cmd(cmd)
