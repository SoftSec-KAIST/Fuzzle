...
def get_TTE(data):
    ...
    for row in data:
        if row['Time taken to first crash'] != None and row['Time taken to first crash'] != '':
            TTE_list.append(float(row['Time taken to first crash']))
    if len(TTE_list) < 1:
        return '-'
    else:
        average_TTE = (statistics.mean(TTE_list))/3600
        return '%02.3f' % average_TTE
...
def get_stdev(data):
    ...
    if len(TTE_list) < 1:
        ...
        stdev = (statistics.stdev(TTE_list))/3600
        return '%02.3f' % stdev
