...
def get_targets(maze_file, numb_targets):
    with open(maze_file + "_" + str(numb_targets) + "_targets.txt", 'r') as f_targets:
...
def get_functions(width, height, maze_exit):
    ...
    if maze_exit == width*height - 1:
        xy_to_func[(height, width-1)] = 'bug_' + str(maze_exit)
    return xy_to_func
...
def generate_graph(width, height, maze_exit, maze_functions, matrix, targets):
    graph = DirGraph()
    functions = list(range(width*height))
    for idx in range(width*height):
        x, y = functions[idx] // width, functions[idx] % width
        node = maze_functions[(x, y)]
        # For now, assume maze exit is always at the bottom right corner
        #if node == maze_exit and node != width*height - 1:
        #    graph.add_edge(node, 'bug')
        if node in targets and node != width*height - 1:
            graph.add_edge(node, 'bug_' + str(node))
        i, j = 2*x + 1, 2*y + 1
        if matrix[i-1][j] == '0':
...
def render_program(c_file, maze, maze_funcs, width, height, generator, sln, smt_file, targets):
    f = open(c_file, 'w')
    ...
    for target in targets:
        f.write("""void func_bug_{}(signed char *input, int index, int length){{ {} }}\n""".format(target, bug))
    f.write(logic_def)
...
def main(maze_file, width, height, numb_targets, cycle, seed, generator, smt_file, CVE_name):
    matrix = get_maze(maze_file)
    sln = get_solution(maze_file)
    targets = get_targets(maze_file, numb_targets)
    maze_exit = get_exit(sln)
    maze_funcs = get_functions(width, height, maze_exit)
    graph = generate_graph(width, height, maze_exit, maze_funcs, matrix, targets)
    remove_cycle(graph, cycle, seed)
    c_file = maze_file + "_" + str(numb_targets) + "_" + str(cycle) + "percent_" + CVE_name + ".c"
    render_program(c_file, graph, maze_funcs, width, height, generator, sln, smt_file, targets)

if __name__ == '__main__':
    maze_file = sys.argv[1]
    width, height = int(sys.argv[2]), int(sys.argv[3])
    numb_targets = int(sys.argv[4])
    ...
    main(maze_file, width, height, numb_targets, cycle, seed, generator, smt_file, CVE_name)
